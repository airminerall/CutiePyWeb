...
function getAccounts() {
    let storedAccounts;
    try {
        storedAccounts = JSON.parse(localStorage.getItem(accountsStorageKey) || '{}');
    } catch {
        storedAccounts = {};
    }
    const accounts = Object.entries(storedAccounts || {}).reduce((normalizedAccounts, [key, account]) => {
        const email = typeof account?.email === 'string' ? account.email.trim().toLowerCase() : key.trim().toLowerCase();
        if (email) normalizedAccounts[email] = { ...account, email };
    ...
    accounts[legacyAccount.email.trim().toLowerCase()] = legacyAccount;
...
form.addEventListener('submit', event => {
    ...
    if (authMode === 'signup') {
        ...
        localStorage.setItem(accountsStorageKey, JSON.stringify(accounts));
        localStorage.setItem(`${progressStorageKey}:${email}`, JSON.stringify({ completedQuestions: {} }));
        passwordInput.value = '';
        const restoreButton = CutiePyLoader.setButtonLoading(submitButton);
        window.setTimeout(() => {
            restoreButton();
            setAuthMode('login');
    ...
    CutiePyLoader.setButtonLoading(submitButton);
    submitButton.style.background = '#34c759';

    setTimeout(() => {
        CutiePyPageProgress.navigate('index.html');
    }, 600);
});
